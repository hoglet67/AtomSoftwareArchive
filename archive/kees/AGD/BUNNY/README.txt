...
Bunny vs Fox - by Kaeleigh and Emily Le. (Unfortunately this game has a glitch which destroyed the text. The girls coded it to say Press 1 by placing blocks which was clever. The controls are WASD with space to jump! 
...
Tape version:
  ...
  BUNNY.CSW, Tapefile for Atomulator, to start the game, type: *RUN"AGD"

Disk version:

  BUNNY.DSK, Diskfile for emulators, to start the game, type *RUN"BURUN"

AtoMMC version:

  BURUN  = Basic introscreen
  BUSCR  = Titlescreen
  BUCODE = Gamecode

  To start the game, type: *BURUN
